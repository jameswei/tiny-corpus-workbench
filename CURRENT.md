...
This checkout identifies package version `0.9.3`. The current product
milestones cover:
...
10. a bilingual learning docsite that teaches the lifecycle through paired
    principles and Workbench practice; and
11. bilingual repository entry points and a localized project landing page
    that present the same learner-oriented lifecycle and interfaces.
...
The current source contains bilingual repository entry points, a localized
project landing page, and the bilingual learning docsite. The CLI remains the
full lifecycle interface. The bundled browser groups immutable records by
document and preparation round. The docsite teaches those concepts and uses
the Workbench as its practice table. For published versions and availability,
use [GitHub Releases](https://github.com/jameswei/tiny-corpus-workbench/releases).
